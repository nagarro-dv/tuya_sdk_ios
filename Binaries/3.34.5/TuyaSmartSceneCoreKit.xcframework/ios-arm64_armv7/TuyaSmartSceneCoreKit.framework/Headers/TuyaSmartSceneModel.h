...
typedef NS_ENUM(NSUInteger, TuyaSmartSceneWorkingStatus) {
    /// Normal state
    TuyaSmartSceneWorkingStatusFine = 0,
    /// Invalid status, scenario cannot be executed successfully
    TuyaSmartSceneWorkingStatusInvalid,
    /// An abnormal state indicates that a device is offline, but the scenario can still be executed
    TuyaSmartSceneWorkingStatusException,
};
...
/// Describe scenario execution status，See enum `TuyaSmartSceneWorkingStatus`.
@property (nonatomic, assign) TuyaSmartSceneWorkingStatus outOfWork;
